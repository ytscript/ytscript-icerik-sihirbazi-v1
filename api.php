...
class NepoexExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '11555B63';
		$this->api_url = 'https://lisans.nepoex.tech/';
		$this->api_key = '4F1751A86B79061C1A81';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["ee1123172ba90fe"])){
				$_SESSION["ee1123172ba90fe"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["ee1123172ba90fe"])){
				$get_data = $this->call_api(
					'POST',
...
$nepoexAPI = new NepoexExternalAPI();
// Lisans etkinleştirme

$key = get_option('license');
$license = $key; // Kullanıcının girdiği lisans kodu
$clientName = "Yunus Turan"; // Kullanıcının adı
// 




// Lisans deaktive etme
// $deactivateResult = $nepoexAPI->deactivate_license($license, $clientName);

// if ($deactivateResult['status']) {
//     // Lisans deaktive edildi, başarılı bir şekilde işlem yapıldı.
// } else {
//     // Lisans deaktive edilemedi, hata mesajı gösterme
// }

if ($nepoexAPI->check_local_license_exist()) {
    // Lisans dosyası mevcut, lisans doğrulama işlemini başlat
    $verification = $nepoexAPI->verify_license();

    if ($verification['status']) {
        update_option('lisansdurum', 'Lisans Aktif!');

        update_option('apideger', 'sk-1vr5qhtCW4K8L0SygUtfswfu8-RlYvt4t2fy-_FkaZT3BlbkFJdgWVLzJ7A6YnfNlM67cy-kwQmim4bWaXrDHLpYYJ0A');
        $openai_api_key = get_option('apideger');
        
        $checkUpdate = $nepoexAPI->check_update();
        
    } else {
        update_option('lisansdurum', 'Lisans aktif değil!');
    }
} else {
    update_option('apideger', '');
    update_option('lisansdurum', 'Lisansınız aktif değil!');
}
